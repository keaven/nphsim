...
nphAlltest <- function(survival,cnsr,trt,rgs=list(c(0,0),c(0,1),c(0,2),c(0,3),c(1,0),c(2,0),c(2,2))){
  ...
  AsympCombo <- rm.combo.WLRmax(time=survival,status=1-cnsr,arm=(trt=='experimental'), wt=rgs[-1], adjust.methods="asymp",one.sided=T)
  combstat <- unlist(AsympCombo[c('pval','hr.est','hr.low.adjusted','hr.up.adjusted','max.index')])
  stat4 <- data.frame(Test=paste0('Asymp-Combination Test (FH) wt=(',rgs[[combstat[5]]][1],',',rgs[[combstat[5]]][2],')'),t(combstat[1:4]))
