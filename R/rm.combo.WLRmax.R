...
)
{
  ...
  if(max & adjust.methods == "asymp"){
    ...
    if(one.sided){pval2 <-1 - pmvnorm(lower = rep(-z.max, length(Z.tst.rslt1)), upper= rep(z.max, length(Z.tst.rslt1)), corr= cor.tst, algorithm= Miwa())[1]
    ...
    if(!one.sided){pval <- 1 - pmvnorm(lower = rep(-z.max, length(Z.tst.rslt1)), upper= rep(z.max, length(Z.tst.rslt1)), corr= cor.tst, algorithm= Miwa())[1]}
  ...
  if(HT.est & max & adjust.methods == "asymp"){
    ...
    for(i in 1:nmodel){
      ...
      Bcor <- cov2cor(Bcov)
      prob.selection[i] <- pmvnorm(lower=rep(0, length(all.cindex)), upper=Inf, corr=Bcor)
...
rm.combo.WLRmax2<- function(time        = NULL,
                           status      = NULL,
                           arm         = NULL,
                           wt          = NULL,
                           adjust.methods = c("holm", "hochberg", "hommel", "bonferroni","asymp")[3],
...
)
{
  ...
  if(HT.est & max & adjust.methods == "asymp"){
    ...
    for(i in 1:nmodel){
      ...
      FH.est2 <- coxph(survival::Surv(time, status) ~ arm + offset(wt2), weight=wt,  method= ties.method, data = data.anal.w2)
