...
- a Z-value corresponding to the logrank test (`z`; signed square root of the chi-square logrank test???), 
...
Following this are one-sided p-values for several weighting schemes, all of which will be produced with the only option to include an arbitrary sequence of $\rho$ and $\gamma$ parameters for the Fleming-Harrington test. 
The tests for which p-values are displayed in this instance are:

- `pval` - ???
- `pval_1` - the logrank test
- `pval_n` - the Gehan-Breslow generalized Wilcoxon test
- `pval_sqrtN - the Tarone-Ware test
- `pval_S1` - the Peto and Peto modified survival test (??? how is this best said with 'test' at the end?)
- `pval_S2` - the Andersen-modified Peto and Peto test
- `pval_APPLE` - the simplified APPLE method with user-specified late separation time specified in `APPLE=1` above
